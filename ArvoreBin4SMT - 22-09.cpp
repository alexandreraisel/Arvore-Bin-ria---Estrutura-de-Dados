...
int desbalanceamentoEsq(arvore *aux) {
	if (aux==NULL) 
       return 0;
   else {
    	int de = desbalanceamentoEsq(aux->esq);
		int dd = desbalanceamentoEsq(aux->dir);
    	de++;
		return(de);
	}
}

int desbalanceamentoDir(arvore *aux) {
	if (aux==NULL) 
       return 0;
    else {
    	int dd = desbalanceamentoDir(aux->dir);
   		int de = desbalanceamentoDir(aux->esq);
   		dd--;
		return(dd);
	}
}

int desbalanceamentoEleEsq(arvore *aux) {
	if (aux==NULL) 
       return 0;
   else {
    	int de = desbalanceamentoEleEsq(aux->esq);
		int dd = desbalanceamentoEleEsq(aux->dir);
    	de++;
		return(de);
	}
}

int desbalanceamentoEleDir(arvore *aux) {
	if (aux==NULL) 
       return 0;
   else {
    	int de = desbalanceamentoEleDir(aux->esq);
		int dd = desbalanceamentoEleDir(aux->dir);
    	dd--;
		return(dd);
	}
}
...
main(){
	...
	int op;
	do {
		printf("\nINFORMA A OPÇÃO:\n1 - ADICIONAR\n2 - DESLOCAR-SE A ESQUERDA\n3 - DESLOCAR-SE A DIREITA\n4 - VOLTAR PARA CIMA\n5 - EXCLUIR\n6 - PRÉ-ORDEM\n7 - PÓS ORDEM\n8 - EM ORDEM\n9 - NÍVEIS DA ÁRVORE\n10 - DESBALANCEAMENTO DA RAIZ\n11 - DESBALANCEAMENTO POR ELEMENTO\n0 - SAIR\n");
		scanf("%d", &op);
		if (op == 1){
			inserir();
		} else if (op == 2){
			...
			if(raiz==NULL) {
				...
				printf("DESBALANCEAMENTO DA RAIZ %d \n", desbalanceamentoEsq(raiz)+desbalanceamentoDir(raiz));
				printf("\n-----------------------------------\n");
			}
		} else if (op==11){
			if(raiz==NULL) {
				...
				printf("DESBALANCEAMENTO DO ELEMENTO %d \n", desbalanceamentoEleEsq(atual)+desbalanceamentoEleDir(atual));
				printf("\n-----------------------------------\n");
			}
		}
	} while (op!=0);
